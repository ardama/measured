...
const History = () => {  
  ...
  const measurementPalette = getCombinedPalette(selectedMeasurement?.baseColor);
  const s = createStyles(theme, measurementPalette);

  const measurementItems: BottomDrawerItem<string>[] = measurements.map(({ id, name: activity, variant, type }) => {
    return {
  ...
  const renderHabitChartCard = (): JSX.Element | null => {
    ...
    const habitChartInputs = [habits, measurements, habitBucketSize.value, habitTrendline.value, habitChartDuration.value, globalPalette.primary];
  ...
  }).filter((data) => data !== null), [selectedMeasurementRecordingDates, today, selectedMeasurement]);
  const selectedVisibleData = selectedMeasurementData.filter(({ x }) => x >= 0);

  const averageValues = useMemo(() =>
    movingAverage(selectedMeasurementData.map(({ y }) => y), measurementTrendline.value),
    [id, measurements, measurementTrendline.value]
  );
        ...
        <View style={s.cardRow}>
          {isFocused ? (
            <View style={{ paddingTop: 52, paddingBottom: 0 }}>
              ...
              {selectedMeasurementDataIndex < 0 ? null : (() => {
